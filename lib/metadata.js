...
function Metadata(features, search, dataset) {
     ...
     */
    var metadata = {};
    var recordId = metadata.recordId = dataset + '!metadata!' + dataset;
    var key = metadata.key = { index: recordId };
    ...
    metadata.calculateInfo = function(callback) {
        ...
        search.queryStream(params)
            .on('error', callback)
            .pipe(through({ objectMode: true }, function(searchItem, enc, cb) {
                features.getItem({Key: {index: searchItem.index.replace('feature_id', searchItem.dataset)}}, function(err, item) {
                    var data = item.Item;
