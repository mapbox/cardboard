...
function Metadata(dyno, dataset) {
    ...
    metadata.defaultInfo = function(callback) {
        ...
        var params = {
            ...
            ExpressionAttributeNames: {
                ...
                '#c': 'count', '#size': 'size', '#u': 'updated', '#id': 'id'
            },
            ExpressionAttributeValues: {
                ':w': 180, ':s': 90, ':e': -180, ':n': -90,
                ':c': 0, ':size': 0, ':u': +new Date()
            },
            UpdateExpression: 'set #w = :w, #s = :s, #e = :e, #n = :n, #c = :c, #size = :size, #u = :u',
    ...
    metadata.calculateInfo = function(callback) {
        ...
        dyno.queryStream(params)
            ...
            .on('error', callback)
            .on('finish', function() {
                dyno.putItem({ Item: info }, function(err) {
    ...
    metadata.adjustProperties = function(properties, callback) {
        var params = {
            Key: key,
            ExpressionAttributeNames: { '#id': 'id', '#u': 'updated' },
            ExpressionAttributeValues: { ':u': +new Date() },
            UpdateExpression: 'set #u = :u add',
