...
module.exports = function(config) {
     ...
     * @static
     * @memberof cardboard.batch
     * @param {string[]} ids - an array of feature ids to retrieve
     * @param {string} dataset - the name of the dataset that these features belongs to
     * @param {function} callback - the callback function to handle the response
     */
    batch.get = function(ids, dataset, callback) {
        
        config.dyno.getItems(ids, null, function(err, data){
            if (err) callback(err);
            utils.resolveFeatures(data, function(err, featureCollection) {
                if (err) return callback(err);
