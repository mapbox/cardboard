...
function Utils(config) {
    ...
    utils.createFeatureKey = function(dataset, featureId) {
      return {
        index: dataset + '!feature!' + featureId
      }
    };
    ...
    utils.resolveFeaturesByIds = function(dataset, featureIds, callback) {
        ...
        featureIds.forEach(function (id) {
            ...
            q.defer(function(done) {
                var params = {Key: utils.createFeatureKey(dataset, id)};
    ...
    utils.toDatabaseRecord = function(feature, dataset) {
        ...
        var info = Metadata(null, null, null).getFeatureInfo(f);
        ...
        var databaseFeature = utils.createFeatureKey(dataset, primary);
        databaseFeature.val = useS3 ? null : buf,
        databaseFeature.size = info.size,
        databaseFeature.west = truncateNum(info.west),
        databaseFeature.south = truncateNum(info.south),
        databaseFeature.east = truncateNum(info.east),
        databaseFeature.north = truncateNum(info.north),
        databaseFeature.s3url = useS3 ? ['s3:/', config.bucket, s3Key].join('/') : null
